...
/** 堆栈帧指针*/
struct YJStackFramePointer {
    const struct YJStackFrameEntry *const previous;
    const uintptr_t address;
};
typedef struct YJStackFramePointer YJStackFramePointer;

@interface YJThreadLogger ()

@property (nonatomic) mach_port_t currentThread; ///< 当前线程

@end

@implementation YJThreadLogger

#pragma mark - init
- (instancetype)init {
    self = [super init];
    if (self) {
        self.currentThread = mach_thread_self();
    }
    return self;
}
...
#pragma mark - private
- (NSString *)logWithThread:(thread_t)thread {
    // 代码借鉴 https://github.com/bestswifter/BSBacktraceLogger
    ...
    for(; i < 50; i++) {
        backtraceBuffer[i] = frame.address;
    ...
    for (int i = 0; i < backtraceLength; ++i) {
        [log appendFormat:@"%@", bs_logBacktraceEntry(i, backtraceBuffer[i], &symbolicated[i])];
    }
    [log appendString:@"}"];
    return log;
}
...
NSString* bs_logBacktraceEntry(const int entryNum, const uintptr_t address, const Dl_info* const dlInfo) {
    char faddrBuff[20];
    char saddrBuff[20];
    ...
    uintptr_t offset = address - (uintptr_t)dlInfo->dli_saddr-1;
